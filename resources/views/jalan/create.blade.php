    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-2 gap-6">
                    ...
                    <div class="p-6">
                        <form method="POST" action="{{ route('jalan.store') }}">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="NamaJalan" value="{{ old('NamaJalan') }}" required 
                                    ...
                                    <select name="StartPointID" id="startPoint" required class="w-full border rounded px-3 py-2">
                                        ...
                                        @foreach($hotels as $hotel)
                                            <option value="{{ $hotel->PointID }}"
                                                ...
                                                {{ old('StartPointID') == $hotel->PointID ? 'selected' : '' }}>
                                                {{ $hotel->NamaObjek }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="EndPointID" id="endPoint" required class="w-full border rounded px-3 py-2">
                                        ...
                                        @foreach($hotels as $hotel)
                                            <option value="{{ $hotel->PointID }}"
                                                data-lat="{{ $hotel->Latitude }}" 
                                                data-lng="{{ $hotel->Longitude }}"
                                                {{ old('EndPointID') == $hotel->PointID ? 'selected' : '' }}>
                                                {{ $hotel->NamaObjek }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" step="0.01" name="DistanceKM" id="distanceKM" value="{{ old('DistanceKM') }}" required 
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="KoordinatJSON" id="koordinatJSON" rows="3" required 
                                    class="w-full border rounded px-3 py-2 font-mono text-xs bg-gray-50" readonly>{{ old('KoordinatJSON') }}</textarea>
                                <p class="text-xs text-gray-500 mt-1">Draw route on map</p>
                                @error('KoordinatJSON')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4 p-3 bg-blue-50 rounded border border-blue-200">
                                ...
                                <ul class="text-xs text-blue-700 list-disc list-inside space-y-1">
                                    <li><strong>Select start point</strong> from dropdown to begin</li>
                                    ...
                                    <li><strong>Click "Finish Route"</strong> to add end point</li>
                                </ul>
                            </div>
                            ...
                            <div class="flex justify-end gap-2">
                                ...
                                <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">Create Route</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold mb-4">Draw Route on Map</h3>
            ...
            setTimeout(function() {
                ...
                function finishRoute() {
                    ...
                    if (!isNaN(lat) && !isNaN(lng)) {
                        ...
                        updateJSON();
                        isDrawing = false;
                        alert('Route completed! You can now create the route.');
                    }
                }
